...
public class MainDay5 {
	...
	public static void convertInput(boolean isV1) throws Exception {
		...
		generateConversionMaps(isDebug);
	...
	private static void generateConversionMaps(boolean isDebug) {
...
//			System.out.println("convertSeedToLocation: conversion " + conversionMapName + " - source=" + seedStates[indexState - 1] + " destination=" + seedStates[indexState]);
	...
	/**
	 * Version alternative pour diviser l'exécution entre les lots de graines plutôt que sur la liste entière
	 * @return
	 * @throws Exception 
	 */
	public static long V2() throws Exception {
		long minLocation = Long.MAX_VALUE;
		long minLocationOfRange;
		
		//Stocker les intervalles
		long[][] tabSeedsRanges;
		
		if(isDebug) {
			 tabSeedsRanges = convertInputSeedsRanges(InputDay5_Test.INPUT_DAY5_SEEDS);
		} else {
			 tabSeedsRanges = convertInputSeedsRanges(InputDay5.INPUT_DAY5_SEEDS_SHORT);
		}
		
		//Générer les tables de conversion	
		generateConversionMaps(isDebug);
		
		//Parcourir chaque intervalles, et récupérer la valeur minimale pour chaque intervalle
		for(int indexSeedRange = 0; indexSeedRange < tabSeedsRanges.length; indexSeedRange++) {
			minLocationOfRange = findLowestLocationNumberV2(tabSeedsRanges[indexSeedRange][0], tabSeedsRanges[indexSeedRange][1], (int) tabSeedsRanges[indexSeedRange][2]);
			
			System.out.println("V2 location minimale pour intervalle {start, size} {" + tabSeedsRanges[indexSeedRange][0] + ", " + tabSeedsRanges[indexSeedRange][1] + "} = " + minLocationOfRange);
			
			//Déterminer la location minimale
			if(minLocation > minLocationOfRange)
				minLocation = minLocationOfRange;
		}
	 ...
	 * Convertit toutes les graines vers leur location, puis détermine le nombre le plus faible
	 */
	public static long findLowestLocationNumberV2(long rangeStart, long rangeEnd, int rangeSize) {
		seedsStateForCategories = new long[rangeSize][8];
		
		long[] seedStates;
		
		long minLocation = Long.MAX_VALUE;
		
		for(long seedValue = rangeStart; seedValue <= rangeEnd; seedValue ++) {
			seedStates = convertSeedToLocation(seedValue);
	 ...
	 * V2: chaque paire de nombres correspond à une intervalle de graines (départ, longueur)
	 * @param seedsLine : tableau des graines
	 * @return tableau {{départ de l'intervalle, fin l'intervalle, taille de l'intervalle}, ...}
	 */
	private static long[][] convertInputSeedsRanges(String seedsLine) throws IllegalArgumentException{
		if(!seedsLine.startsWith("seeds: "))
			throw new IllegalArgumentException("convertInputSeedsRanges(): L'input n'est pas la liste des graines - input: '" + seedsLine + "'");
		
		String[] seeds = seedsLine.substring(7).split(" ");
		
		long[][] seedsRanges = new long[seeds.length / 2][3];
		
		int indexSeedsRanges = 0;
		
		for(int indexSeed = 0; indexSeed < seeds.length; indexSeed += 2) {
			//Version {départ, taille}
			/*seedsRanges[indexSeed][0] = Long.parseLong(seeds[indexSeed]);
			seedsRanges[indexSeed][1] = Long.parseLong(seeds[indexSeed] + 1);*/
		
			//Version {départ, fin}
			seedsRanges[indexSeedsRanges][0] = Long.parseLong(seeds[indexSeed]);
			seedsRanges[indexSeedsRanges][1] = Long.parseLong(seeds[indexSeed]) + Long.parseLong(seeds[indexSeed + 1]) - 1;
			seedsRanges[indexSeedsRanges][2] = Long.parseLong(seeds[indexSeed + 1]);
